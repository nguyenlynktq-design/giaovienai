...
const SettingsModal: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-pink-100 flex flex-col max-h-[90vh]">

                {/* Header */}
                <div className="bg-gradient-to-r from-rose-500 to-pink-500 px-6 py-4 flex justify-between items-center flex-none">
                    ...
                    <div className="mb-8">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-rose-500 focus:border-rose-500 outline-none transition text-slate-800 font-mono"
                            />
                        </div>
                    </div>
                            ...
                            {AVAILABLE_MODELS.map((model) => (
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        <span className={`font-bold text-sm ${tempModel === model.id ? 'text-rose-900' : 'text-slate-700'}`}>
                                            {model.name}
                                        </span>
                                        {tempModel === model.id && <Check className="w-5 h-5 text-rose-600" />}
                                    </div>
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 flex-none">
                    ...
                    <button
                        onClick={handleSave}
                        className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-pink-500 text-white font-bold hover:from-rose-600 hover:to-pink-600 active:scale-[0.98] transition shadow-md shadow-rose-200"
                    >
                        Lưu cài đặt
                    </button>
                </div>
